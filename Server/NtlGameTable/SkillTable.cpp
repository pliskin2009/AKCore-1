...
bool CSkillTable::AddTable(void * pvTable, bool bReload)
{
...
bool CSkillTable::SetTableData(void* pvTable, WCHAR* pwszSheetName, std::wstring* pstrDataName, BSTR bstrData)
{
	if (0 == wcscmp(pwszSheetName, L"Table_Data_KOR"))
	{
		...
		{
			pSkill->wFunction_Bit_Flag = (WORD)(READ_BITFLAG(bstrData));
		}
		...
		{
			pSkill->fSkill_Effect_Value[0] = READ_FLOAT(bstrData, pstrDataName->c_str());
		}
		...
		{
			pSkill->fSkill_Effect_Value[1] = READ_FLOAT(bstrData, pstrDataName->c_str());
		}
		else if( 0 == wcsncmp(pstrDataName->c_str(), L"RP_Effect_", wcslen(L"RP_Effect_") ) )
		{
			...
			for (BYTE byRpEffectIndex = 0 ; byRpEffectIndex < DBO_MAX_RP_BONUS_COUNT_PER_SKILL ; byRpEffectIndex++)
			{
				...
				{
					(pSkill->adwRpEffectValue)[byRpEffectIndex] = READ_DWORD(bstrData);
				}
			}
		}
		...
		{
			pSkill->wRequire_LP = READ_WORD(bstrData, pstrDataName->c_str(), 0);
		}
		...
		{
			pSkill->wAdditional_Aggro_Point = READ_WORD(bstrData, pstrDataName->c_str(), 0);
		}
...
bool CSkillTable::LoadFromBinary(CNtlSerializer& serializer, bool bReload)
{
	if( false == bReload )
	{
		Reset();
