...
bool CSpawnTable::AddTable(void * pvTable, bool bReload)
{
	...
	if( INVALID_SPAWNGROUPID != pTbldat->spawnGroupId )
	{
		...
		{
			CTable::CallErrorCallbackFunction(L"Tblidx[%u]::Spawn_Group[%u] is Duplicated ", pTbldat->tblidx, pTbldat->spawnGroupId );
			_ASSERTE( 0 );
			return false;
		}
	}
...
bool CSpawnTable::LoadFromBinary(CNtlSerializer& serializer, bool bReload)
{
	if( false == bReload )
	{
		Reset();
