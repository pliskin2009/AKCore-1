...
#define NTL_MAX_SIZE_QUESTION_CONTENT_UNICODE					(256)
#define NTL_MAX_SIZE_QUESTION_CONTENT_BUFFER_MULTIBYTE			(512)

#define NTL_MIN_SIZE_CUSTOMER_SATISFACTION						(4)
#define NTL_MAX_SIZE_CUSTOMER_SATISFACTION						(512)
#define NTL_MAX_SIZE_CUSTOMER_SATISFACTION_UNICODE				(512)
#define NTL_MAX_SIZE_CUSTOMER_SATISFACTION_BUFFER_MULTIBYTE		(1024)

#define NTL_MIN_SIZE_PETITION_RESULTTEXT						(4)
#define NTL_MAX_SIZE_PETITION_RESULTTEXT						(512)
#define NTL_MAX_SIZE_PETITION_RESULTTEXT_UNICODE				(512)
#define NTL_MAX_SIZE_PETITION_RESULTTEXT_BUFFER_MULTIBYTE		(1024)

#define NTL_MIN_SIZE_NOTE										(0)
#define NTL_MAX_SIZE_NOTE										(512)
#define NTL_MAX_SIZE_NOTE_UNICODE								(512)
#define NTL_MAX_SIZE_NOTE_BUFFER_MULTIBYTE						(1024)
...
#define NTL_MAX_SIZE_STARTGMID_UNICODE							(256)// 대응 GM ID
#define NTL_MAX_SIZE_STARTGMID_BUFFER_MULTIBYTE					(512)

#define NTL_MAX_SIZE_CHANGECATEGORYGMID							(256)// 전달 GM ID
#define NTL_MAX_SIZE_CHANGECATEGORYGMID_UNICODE					(256)// 전달 GM ID
#define NTL_MAX_SIZE_CHANGECATEGORYGMID_BUFFER_MULTIBYTE		(512)

#define NTL_MAX_SIZE_LOG_TEXT									(1024)
#define NTL_MAX_SIZE_LOG_TEXT_UNICODE							(1024)
#define NTL_MAX_SIZE_LOG_TEXT_BUFFER_MULTIBYTE					(2048)

#define NTL_MAX_SIZE_PETITIONCOUNSEL_USERID						( NTL_MAX_SIZE_USERID + NTL_MAX_SIZE_CHAR_NAME )
#define NTL_MAX_SIZE_PETITIONCOUNSEL_USERID_UNICODE				( NTL_MAX_SIZE_USERID_UNICODE + NTL_MAX_SIZE_CHAR_NAME_UNICODE )
#define NTL_MAX_SIZE_PETITIONCOUNSEL_USERID_BUFFER_MULTIBYTE	( NTL_MAX_SIZE_USERID_MULTIBYTE_BUFFER + NTL_MAX_SIZE_CHAR_NAME_BUFFER_MULTIBYTE )
...
struct sPETITION_PROGRESSSTATE_DATA
{
	...
	WCHAR							awchGMUserId[NTL_MAX_SIZE_USERID_UNICODE  + 1];
	WCHAR							awchPetitionCharName[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1];
	...
	union uDATA
	{
		...
		struct sSAY // Start, End, GM Say, User Say
		{
			...
			WCHAR				wszPetitionSay[NTL_MAX_LENGTH_OF_CHAT_MESSAGE_UNICODE + 1];
		} sSay;
...
struct sPETITIONLISTDATA
{
	...
	WCHAR									awchUserCharacterName_PetitionCreate[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1];
	WCHAR									awchGMCharacterName_PetitionCreate[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1]; // 없애자
...
struct sPETITIONDATA
{
	...
	WCHAR									awchUserAccount_PetitionCreate[NTL_MAX_SIZE_USERID_UNICODE + 1];// Petition 생성당시 유져의 Account ID ( 무조건 채워진다 )
	WCHAR									awchUserCharacterName_PetitionCreate[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1];// Petition 생성당시 유져의 Char ID ( 무조건 채워진다 )
	WCHAR									awchGMAccount_PetitionCreate[NTL_MAX_SIZE_USERID_UNICODE + 1];// Petition을 생성한 GM의 Account ID ( 유져 생성시 "" )
	WCHAR									awchGMCharacterName_PetitionCreate[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1];// Petition을 생성한 GM의 Account ID ( 유져 생성시 "" )

	WCHAR									awchQuestionContent[NTL_MAX_SIZE_QUESTION_CONTENT_UNICODE + 1]; // 문의 내용
	eCUSTOMERSATISFACTION					eCustomerSatisfaction; // 고객 만족도
	WCHAR									awchPetitionResultText[NTL_MAX_SIZE_PETITION_RESULTTEXT_UNICODE + 1]; // 상담 결과
	WCHAR									awchNote[NTL_MAX_SIZE_NOTE_UNICODE + 1]; // 비고 ( 보류할때 쓰는 비고 )
	...
	WCHAR									awchBlackList[ NTL_MAX_LENGTH_OF_CHAT_MESSAGE_UNICODE + 1];

	//bool									bIsConnectUser;
	//SERVERFARMID							serverfarmID_ConnectUser;
	//WCHAR									awchUserCharacterName_ConnectUser[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1];
	//CHARACTERID							characterID_ConnectUser;

	GMACCOUNTID								gmAccountID_Consult; // 대응 정보의 GM Account ID( 시리얼 )
	WCHAR									awchGMAccount_Consult[NTL_MAX_SIZE_USERID_UNICODE + 1]; // 대응 상황의 GM Account ( Char )
	WCHAR									awchGMCharName_Consult[NTL_MAX_SIZE_CHAR_NAME_UNICODE + 1];// 대응 상황의 GM 이름

	WCHAR									awchReserveGMID[NTL_MAX_SIZE_RESERVEGMID_UNICODE + 1]; //대응 상황의 보류 GM ID
	WCHAR									awchStartGMID[NTL_MAX_SIZE_STARTGMID_UNICODE + 1]; // 대응 상황의 대응 GM ID
	WCHAR									awchChangeCategoryGMID[NTL_MAX_SIZE_CHANGECATEGORYGMID_UNICODE + 1]; // 대응상황의 카테고리 변경 GM ID
...
struct sPETITION_COUNSELDATA
{
	...
	WCHAR									awchAccount[NTL_MAX_SIZE_PETITIONCOUNSEL_USERID_UNICODE + 1];
	WCHAR									awchText[NTL_MAX_SIZE_LOG_TEXT_UNICODE + 1];
